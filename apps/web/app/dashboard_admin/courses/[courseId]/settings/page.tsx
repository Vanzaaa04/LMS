  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Lecturers
        const lecRes = await fetch(buildApiUrl('/admin/users?role=LECTURER'), {
        ...
        if (courseRes.ok) {
          ...
          setDraft({
            title: course.title || '',
            semester: course.semester || 'Fall Semester 2026',
            credits: course.credits?.toString() || '3',
            enrollmentCap: course.enrollmentCap?.toString() || '60',
      ...
      <main className="main-content">
        <header className="top-bar">
          <div>
            ...
            <p className="page-subtitle">Ubah pengaturan mata kuliah.</p>
          </div>
        </header>
        <div className="dashboard-content" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
            ...
            <div>
              <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>Edit Mata Kuliah</h1>
            </div>
          </div>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '32px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)' }}>
            ...
            <form onSubmit={handleUpdate} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '14px', fontWeight: 600, color: '#334155', marginBottom: '8px' }}>Status Kelas</label>
                <select value={draft.status} onChange={e => setDraft({...draft, status: e.target.value})} style={{ width: '100%', padding: '10px 14px', borderRadius: '8px', border: '1px solid #CBD5E1', background: 'white' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '16px' }}>
                ...
                <button type="submit" disabled={saving} style={{ padding: '10px 24px', background: 'var(--primary)', color: 'white', borderRadius: '8px', fontWeight: 600, border: 'none', fontSize: '14px', cursor: saving ? 'not-allowed' : 'pointer' }}>
